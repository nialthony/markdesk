 ...
 *
 * The protocol layer needs SHA-256 for Anchor account, instruction, and event
 * discriminators. `node:crypto` is unavailable in the browser bundle, and the
 * Web Crypto API is async, so a small synchronous implementation keeps the
 * shared package network- and runtime-independent.
 */
...
export function sha256(data: Uint8Array): Uint8Array {
  ...
  for (let offset = 0; offset < paddedLength; offset += 64) {
    for (let i = 0; i < 16; i += 1) {
      const j = offset + i * 4;
      w[i] =
        padded[j]! * 2 ** 24 + padded[j + 1]! * 2 ** 16 + padded[j + 2]! * 2 ** 8 + padded[j + 3]!;
    }
...
export function sha256Hex(text: string): string {
  return sha256(new TextEncoder().encode(text)).reduce(
