...
const UNKNOWN_EXTENSION_NAMES: Readonly<Record<number, string>> = {
  16: "ConfidentialTransferFeeConfig",
  17: "ConfidentialTransferFeeAmount",
  24: "ConfidentialMintBurn",
};

function publicKey(value: PublicKey | null | undefined): string | null {
  return value?.toBase58() ?? null;
}

function extensionName(value: ExtensionType): string {
...
async function inspectMint(
  ...
  asset: ReturnType<typeof normalizePreStocks>[number],
) {
  const address = new PublicKey(asset.mint);
  const account = await connection.getAccountInfo(address, "confirmed");
  if (!account) throw new Error(`Mint account not found: ${asset.mint}`);
