...
export const SYNTHETIC_MAX_FEE = 0xffff_ffff_ffff_ffffn;
...
function configAddress(programId: PublicKey): PublicKey {
  return PublicKey.findProgramAddressSync([Buffer.from(MARKDESK_SEEDS.config)], programId)[0];
}

function markAddress(baseMint: PublicKey, programId: PublicKey): PublicKey {
  return PublicKey.findProgramAddressSync(
    [Buffer.from(MARKDESK_SEEDS.mark), baseMint.toBuffer()],
    programId,
  )[0];
}
...
export function buildSyntheticBaseMintTransaction(input: {
  ...
  authority: PublicKey;
  lamports: number;
}): { transaction: Transaction; space: number } {
  ...
  transaction.add(
    ...
    createInitializePausableConfigInstruction(input.mint, input.authority, TOKEN_2022_PROGRAM_ID),
    createInitializeDefaultAccountStateInstruction(
      input.mint,
