...
import {
  buildFundWalletTransaction,
  buildInitializeConfigTransaction,
  buildPublishMarkTransaction,
  buildQuoteMintTransaction,
  buildSyntheticBaseMintTransaction,
  devnetConfigAddress,
  devnetMarkAddress,
  SYNTHETIC_BASE_DECIMALS,
  SYNTHETIC_BASE_MINT_SPACE,
  SYNTHETIC_QUOTE_DECIMALS,
  SYNTHETIC_QUOTE_MINT_SPACE,
} from "./ix";
...
async function main(): Promise<void> {
  ...
  if (args.programId.toBase58() === MARKDESK_PROGRAM_ID_PLACEHOLDER) {
    console.warn(
      "Warning: --program is still the source placeholder. Pass the real program id after `anchor keys sync` and deployment.",
    );
  }
  ...
  if (!programInfo || !programInfo.executable) {
    console.error(
      `✗ MarkDesk program ${args.programId.toBase58()} is not deployed on ${args.rpcUrl}.\n` +
  ...
  if (args.baseMint) {
    ...
    if (!(await connection.getAccountInfo(baseMint, "confirmed"))) {
      ...
      console.log(
        `· Created synthetic base mint ${baseMint.toBase58()} (Token-2022, 50 bps fee, delegate, pausable, scaled UI 1×) — ${explorerTx(signature)}`,
      );
    } else {
  ...
  if (args.quoteMint) {
    ...
    if (!(await connection.getAccountInfo(quoteMint, "confirmed"))) {
      ...
      console.log(
        `· Created quote mint ${quoteMint.toBase58()} (6 decimals, no fee) — ${explorerTx(signature)}`,
      );
    } else {
  ...
  if (!configInfo) {
    ...
    const signature = await sendPayer(connection, payer.keypair, transaction);
    console.log(
      `· Initialized config ${configKey.toBase58()} (publisher ${publisher.keypair.publicKey.toBase58()}, max mark age ${MAX_MARK_AGE_SECONDS}s) — ${explorerTx(signature)}`,
    );
  } else {
    ...
    if (!configuredQuote.equals(quoteMint)) {
      console.error(
        `✗ Config already exists with quote mint ${configuredQuote.toBase58()}.\n` +
  ...
  const mintRaw = (tokens: number, decimals: number) => BigInt(tokens * 10 ** decimals);
  const fund = async (
    owner: PublicKey,
    mint: PublicKey,
    tokens: number,
  ...
  if (args.fundAddress) {
    await fund(
      ...
      10,
      baseDecimals,
      `extra wallet base ${args.fundAddress.toBase58()}`,
    );
    await fund(
      args.fundAddress,
      quoteMint,
      1_000,
      SYNTHETIC_QUOTE_DECIMALS,
      `extra wallet quote ${args.fundAddress.toBase58()}`,
    );
  }

  // 8. Persist state and print the web app configuration.
  const devnetState: DevnetState = {
    cluster: "devnet",
  ...
  console.log(
    `  NEXT_PUBLIC_SOLANA_RPC_URL=${args.rpcUrl === DEFAULT_DEVNET_RPC ? DEFAULT_DEVNET_RPC : args.rpcUrl}`,
  );
