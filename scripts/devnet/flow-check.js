...
import { confirmTransactionWithPayer, ensureSolBalance, explorerTx, keypairSigner, loadOrCreateKeypair, parseDevnetArgs, readDevnetState, } from "./common";
import { buildFundWalletTransaction, buildPublishMarkTransaction, devnetMarkAddress } from "./ix";
import { planCancelOffer, planCreateOffer, planFillOffer, runCancelOfferFlow, runCreateOfferFlow, runFillOfferFlow, } from "../../apps/web/lib/solana/flows";
...
async function main() {
    ...
    if (!state && (!args.programId || !args.baseMint || !args.quoteMint)) {
        console.error("✗ No var/state/devnet.json found. Run `npm run devnet:bootstrap` first, or pass --program, --base-mint, and --quote-mint.");
        process.exit(1);
    }
    ...
    await ensureSolBalance(connection, maker.keypair.publicKey, 150000000n, payer.keypair);
    await ensureSolBalance(connection, taker.keypair.publicKey, 150000000n, payer.keypair);
    await ensureSolBalance(connection, publisher.keypair.publicKey, 50000000n, payer.keypair);
    ...
    async function topUp(owner, mint, targetRaw, label) {
    ...
    const createPlan = planCreateOffer({
        ...
        offerId: BigInt(Date.now()) * 1000n,
    });
    if (!createPlan.ok) {
        console.error("✗ Create blocked: " + createPlan.blockers.join(" | "));
        process.exit(1);
    }
    console.log(`· Create: gross ${createPlan.plan.grossDepositRaw.toString()} raw → vault ${createPlan.plan.vaultCreditRaw.toString()} → buyer net ${createPlan.plan.buyerNetRaw.toString()} → quote ~${createPlan.plan.quoteEstimateRaw.toString()}`);
    ...
    const createPlan2 = planCreateOffer({
        ...
        offerId: BigInt(Date.now()) * 1000n + 1n,
    });
    if (!createPlan2.ok) {
...
function record(name, status, outcome) {
