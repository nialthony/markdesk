...
export function markAddress(baseMint: PublicKey, programId: PublicKey): PublicKey {
...
export function offerAddress(maker: PublicKey, offerId: bigint, programId: PublicKey): PublicKey {
  const offerIdLe = Buffer.alloc(8);
  offerIdLe.writeBigUInt64LE(offerId);
  return PublicKey.findProgramAddressSync(
    [Buffer.from(MARKDESK_SEEDS.offer), maker.toBuffer(), offerIdLe],
    programId,
  )[0];
}

/** The offer's escrow vault: base-mint ATA owned by the Offer PDA. */
export function offerVaultAddress(
  offer: PublicKey,
  baseMint: PublicKey,
...
export function walletTokenAddress(
  owner: PublicKey,
  mint: PublicKey,
...
export function isTokenProgram(program: PublicKey): boolean {
