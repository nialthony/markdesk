import { decodeAnchorEvent, decodeConfigAccount, decodeMarkAccount, decodeOfferAccount, isMarkFresh, } from "@markdesk/core";
import { AccountState, getDefaultAccountState, getExtensionTypes, getPausableConfig, getPermanentDelegate, getScaledUiAmountConfig, getTransferFeeAmount, getTransferFeeConfig, getTransferHook, unpackAccount, unpackMint, TOKEN_2022_PROGRAM_ID, } from "@solana/spl-token";
...
function decodeMintInsights(mintAddress, info, clusterTimestampSeconds, epoch) {
    if (!info)
        return null;
    ...
    const currentTerms = transferFee && epoch >= transferFee.newerTransferFee.epoch
        ? transferFee.newerTransferFee
        : (transferFee?.olderTransferFee ?? null);
    const scheduledTerms = transferFee && epoch < transferFee.newerTransferFee.epoch ? transferFee.newerTransferFee : null;
    const pendingActive = scaled !== null && clusterTimestampSeconds >= Number(scaled.newMultiplierEffectiveTimestamp);
    ...
    return {
        ...
        defaultAccountState: defaultState === AccountState.Frozen
            ? "Frozen"
...
async function readTokenPosition(connection, owner, mint, tokenProgram) {
...
export async function readProtocolState(input) {
    ...
    const [markInfo, baseMintInfo, quoteMintInfo] = await connection.getMultipleAccountsInfo([markAddress(baseMint, cluster.programId), baseMint, ...(quoteMintKey ? [quoteMintKey] : [])], "confirmed");
    ...
    if (wallet) {
        const base = baseInsights
            ? await readTokenPosition(connection, wallet, baseMint, new PublicKey(baseInsights.tokenProgram))
            : null;
        const quote = quoteMint
            ? await readTokenPosition(connection, wallet, new PublicKey(quoteMint.address), new PublicKey(quoteMint.tokenProgram))
            : null;
...
export async function readOfferState(input) {
    ...
    const makerQuote = baseRead.quoteMint
        ? await readTokenPosition(connection, maker, new PublicKey(baseRead.quoteMint.address), new PublicKey(baseRead.quoteMint.tokenProgram))
        : null;
    return {
        readAtMs: Date.now(),
        cluster,
...
export function eventsFromTransaction(response) {
    const events = [];
