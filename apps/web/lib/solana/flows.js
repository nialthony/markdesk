import { applyUiMultiplierAsProgram, calculateGrossDepositForBuyerNetRaw, calculateQuoteRawForScaledAmount, calculateTransferFeeRaw, encodeCancelOffer, encodeCreateOffer, encodeFillOffer, MARKDESK_LIMITS, } from "@markdesk/core";
import { createAssociatedTokenAccountIdempotentInstruction, getTransferFeeAmount, unpackAccount, } from "@solana/spl-token";
import { ComputeBudgetProgram, PublicKey, SystemProgram, Transaction, VersionedTransaction, } from "@solana/web3.js";
import { ASSOCIATED_TOKEN_PROGRAM_ID, configAddress, markAddress, offerAddress, offerVaultAddress, walletTokenAddress, } from "./pdas";
import { eventsFromTransaction, } from "./read";
...
export function planCreateOffer(input) {
    ...
    if (!read.programDeployed)
        blockers.push("MarkDesk program is not deployed on this cluster.");
    if (!config)
        blockers.push("Protocol config is not initialized on this cluster.");
    if (!baseMint)
        blockers.push("Base mint account was not found on this cluster.");
    if (!mark)
        blockers.push("No on-chain mark exists for this base mint yet.");
    if (mark && config && !mark.fresh) {
        blockers.push(`On-chain mark is stale (${mark.ageSeconds}s old, limit ${config.maxMarkAgeSeconds}s).`);
    }
    if (!read.quoteMint)
        blockers.push("Configured quote mint was not found on this cluster.");
    if (baseMint?.pausable?.paused)
        blockers.push("Base mint is paused by its issuer.");
    ...
    if (input.offsetBps < MARKDESK_LIMITS.minOffsetBps ||
    ...
    if (lifetime <= 0)
        blockers.push("Expiry must be in the future.");
    if (lifetime > MARKDESK_LIMITS.maxOfferLifetimeSeconds) {
        blockers.push("Expiry exceeds the 30-day maximum offer lifetime.");
    }
    if (input.buyerNetTargetRaw <= 0n)
        blockers.push("Buyer-net target must be positive.");
    const position = read.wallet?.base ?? null;
    if (!read.wallet)
        blockers.push("Connect a wallet to plan an offer.");
    ...
    const vaultKey = offerVaultAddress(offerKey, new PublicKey(baseMint.address), new PublicKey(baseMint.tokenProgram));
    return {
        ok: true,
        plan: {
...
export function buildCreateOfferTransaction(input) {
    ...
    transaction.add({
        ...
        programId,
        data: Buffer.from(encodeCreateOffer({
...
export function planFillOffer(input) {
    ...
    if (!config)
        blockers.push("Protocol config is not initialized on this cluster.");
    if (!baseMint)
        blockers.push("Offer base mint was not found on this cluster.");
    if (!read.quoteMint)
        blockers.push("Configured quote mint was not found on this cluster.");
    if (!mark)
        blockers.push("No on-chain mark exists for this offer's base mint.");
    if (mark && config && !mark.fresh) {
        blockers.push(`On-chain mark is stale (${mark.ageSeconds}s old, limit ${config.maxMarkAgeSeconds}s).`);
    }
    ...
    if (!read.wallet)
        blockers.push("Connect a wallet to plan a fill.");
...
export function buildFillOfferTransaction(input) {
    ...
    if (plan.needsTakerBaseAta) {
        transaction.add(createAssociatedTokenAccountIdempotentInstruction(taker, takerBase, taker, baseMint, baseTokenProgram));
    }
    if (plan.needsMakerQuoteAta) {
        transaction.add(createAssociatedTokenAccountIdempotentInstruction(taker, makerQuote, maker, quoteMint, quoteTokenProgram));
    }
    transaction.add({
        keys: [
        ...
        programId,
        data: Buffer.from(encodeFillOffer({
...
export function planCancelOffer(input) {
    ...
    const position = read.wallet?.base ?? null;
    if (!read.wallet)
        blockers.push("Connect a wallet to plan a cancellation.");
    if (position && position.amountRaw === null) {
...
export function summarizeLogsError(fallback, logs) {
...
export async function simulateSignSendConfirm(input) {
    ...
    const wire = VersionedTransaction.deserialize(transaction.serialize({ requireAllSignatures: false, verifySignatures: false }));
    ...
    try {
        ...
        const message = error instanceof Error ? error.message : "The RPC node rejected the transaction.";
        return {
            status: "failed",
            stage: "sending",
    ...
    try {
        ...
        if (confirmation.value.err) {
            ...
            return {
                status: "failed",
                stage: "confirming",
                error: summarizeLogsError(`Transaction failed on-chain: ${JSON.stringify(confirmation.value.err)}`, logs),
                logs,
                signature,
            };
        }
...
export async function fetchConfirmedTransaction(connection, signature, attempts = 10) {
...
async function readPosition(connection, owner, mint, tokenProgram) {
...
export async function runCreateOfferFlow(input) {
    ...
    const makerBase = await readPosition(connection, signer.publicKey, new PublicKey(plan.baseMintAddress), new PublicKey(plan.baseTokenProgram));
    const checks = [];
    checks.push(offerInfo !== null
    ...
    if (offerInfo) {
        ...
        const matches = decoded.baseAmount === plan.vaultCreditRaw &&
    ...
    if (plan.preBaseAmountRaw !== null && makerBase.amountRaw !== null) {
        ...
        checks.push({
            ...
            detail: makerBase.amountRaw === expected
    ...
    if (event && "escrowedAmount" in event) {
        const matches = event.escrowedAmount === plan.vaultCreditRaw &&
...
export async function runFillOfferFlow(input) {
    ...
    const [offerInfo, vaultInfo] = await connection.getMultipleAccountsInfo([new PublicKey(plan.offerAddress), new PublicKey(plan.vaultAddress)], "confirmed");
    const taker = signer.publicKey;
    const takerBase = await readPosition(connection, taker, new PublicKey(plan.baseMintAddress), new PublicKey(plan.baseTokenProgram));
    const takerQuote = await readPosition(connection, taker, new PublicKey(plan.quoteMintAddress), new PublicKey(plan.quoteTokenProgram));
    const makerQuote = await readPosition(connection, new PublicKey(plan.maker), new PublicKey(plan.quoteMintAddress), new PublicKey(plan.quoteTokenProgram));
    const checks = [
        {
    ...
    if (takerBase.amountRaw !== null) {
        ...
        checks.push({
            ...
            detail: takerBase.amountRaw === expected
        ...
        if (takerBase.withheldRaw !== null) {
            ...
            checks.push({
                ...
                detail: takerBase.withheldRaw === expectedWithheld
    ...
    if (takerQuote.amountRaw !== null) {
        ...
        checks.push({
            ...
            detail: takerQuote.amountRaw === expected
...
export async function runCancelOfferFlow(input) {
    ...
    if (makerBase.amountRaw !== null) {
        ...
        if (makerBase.withheldRaw !== null) {
            ...
            checks.push({
                ...
                detail: makerBase.withheldRaw === expectedWithheld
    ...
    if (event && "makerNetReturnAmount" in event) {
        const matches = event.makerNetReturnAmount === plan.netReturnRaw &&
